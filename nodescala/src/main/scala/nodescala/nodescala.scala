...
trait NodeScala {
   ...
   *  3) as long as the token is not cancelled and thertrait NodeScalae is a request from the http listener,
  ...
  def start(relativePath: String)(handler: Request => Response): Subscription = {
    ...
    Subscription.apply(subscription, worker)
  }
